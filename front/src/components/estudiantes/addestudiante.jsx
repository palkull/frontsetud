...
import { useParticipantes } from "../../context/ParticipantesContext"; // Ajusta la ruta según tu estructura

function AddParticipantes() {
  const { register, handleSubmit, formState: { errors }, reset } = useForm();
  const navigate = useNavigate();
  const { createParticipante, error } = useParticipantes(); // Asume que tienes este context
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      reset();
      setTimeout(() => navigate("/estudiantes"), 2100);
    } catch (err) {
